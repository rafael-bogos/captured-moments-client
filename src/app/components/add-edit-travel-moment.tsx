...
import { TagInput } from "./card/tag-input";
...
export const AddEditTravelMoment = ({
    ...
    const updateCapturedMoment = async () => {
        ...
        try {
            ...
            console.log("memoryImg: ", memoryImg)

            // verifico se a imagem está prensete
            if (memoryImg && typeof memoryImg !== 'string') {
                const imageUploadResponse = await uploadImage(memoryImg)
                // pegar a URL
                newImageUrl = imageUploadResponse || ""
            ...
            console.log("updateMomentData: ", updateMomentData)
            const response = await axiosInstance.put(`/edit-moment/${momentId}`, updateMomentData)
    ...
    const handleGenerateIA = async () => {
        ...
        try {
            ...
            typeText(response.data.message)
    ...
    const handleDeleteMomentImg = async () => {
        ...
        if (deleteImgResponse.data) {
            ...
            // Updating moment
            await axiosInstance.put(`/edit-moment/${momentId}`, updateMomentData)
        }
    }
            ...
            <header className="flex items-center justify-between">
                ...
                <div>
                    <div className="flex items-center gap-3 bg-violet-50/50 p-2 rounded-l-lg">
                        {type === 'add' ?
                            <button className="flex items-center cursor-pointer gap-1 text-xs font-medium bg-violet-50 text-[#8c52ff] shadow-violet-100/0 border border-violet-100 hover:bg-[#8c52ff] hover:text-white rounded px-3 py-[3px]" onClick={handleUpdateOrAddClick}>
                                <MdAdd /> ADD MOMENT
                            </button>
                            :
                            <button className="flex items-center cursor-pointer gap-1 text-xs font-medium bg-violet-50 text-[#8c52ff] shadow-violet-100/0 border border-violet-100 hover:bg-[#8c52ff] hover:text-white rounded px-3 py-[3px]" onClick={handleUpdateOrAddClick}>
                    ...
                    <div className="flex flex-col gap-2 mt-4">
                        <header className="flex justify-between">
                            ...
                            <button
                                ...
                                className={`border py-1 px-3 rounded-md text-xl cursor-pointer
                                    ${moment && !isIATyping
                                        ? 'bg-slate-50 border-slate-200/50 text-violet-500 hover:bg-[#8c52ff] hover:text-white'
